...
function populateAntibioticOptions() { Object.entries(antibioticCatalog).forEach(([key, antibiotic]) => { const option = document.createElement('option'); option.value = key; option.textContent = antibiotic.name; selectEl.appendChild(option); }); }
...
function recommendedFormulation(antibiotic, weightKg) { const dosePerAdministration = perDoseAmount(antibiotic, weightKg); return antibiotic.formulation.map((option) => { const volumeOrCount = option.concentration > 0 ? dosePerAdministration / option.concentration : 0; return { option, volumeOrCount, difference: Math.abs(volumeOrCount - Math.round(volumeOrCount || 0)) }; }).sort((a, b) => a.difference - b.difference)[0] || { option: antibiotic.formulation[0], volumeOrCount: 0 }; }
...
function getDosesPerDay(item) { const frequency = Number(item.frequency) || 0; if (item.frequencyUnit === 'hours') return frequency > 0 ? 24 / frequency : 0; if (item.frequencyUnit === 'BID') return 2; if (item.frequencyUnit === 'TID') return 3; if (item.frequencyUnit === 'QID') return 4; return frequency; }
function calculateTotalQuantity(item) { const dose = Number(item.dose) || 0; const doseInMg = item.doseUnit === 'ml' ? dose * (Number(item.strength) || 0) : dose; return doseInMg * getDosesPerDay(item) * (Number(item.duration) || 0); }
function createNumberInput(labelText, value, step, onInput) { const label = document.createElement('label'); label.innerHTML = `<span>${labelText}</span>`; const input = document.createElement('input'); input.type = 'number'; input.min = '0'; input.step = step; input.value = value; input.addEventListener('input', () => onInput(input.value)); label.appendChild(input); return { label, input }; }

function buildDosePanel(dosing, headingText) {
  ...
  const frequencyUnit = document.createElement('label'); frequencyUnit.innerHTML = '<span>Schedule</span>'; const frequencyUnitSelect = document.createElement('select'); frequencyUnitSelect.innerHTML = '<option value="hours">q hours</option><option value="BID">BID</option><option value="TID">TID</option><option value="QID">QID</option><option value="times/day">times/day</option>'; frequencyUnitSelect.value = dosing.frequencyUnit;
  ...
  frequency.input.disabled = ['BID', 'TID', 'QID'].includes(dosing.frequencyUnit); const duration = createNumberInput('Duration (days)', dosing.duration, '1', (value) => { dosing.duration = value; updateTotal(); }); panel.appendChild(duration.label); updateTotal(); panel.appendChild(totalBox); return panel;
}

function buildIndicationSelect(antibiotic, selectedItem) { const label = document.createElement('label'); label.className = 'indication-field'; label.innerHTML = '<span>Indication</span>'; const select = document.createElement('select'); select.className = 'indication-select'; antibiotic.indications.forEach((indication) => { const option = document.createElement('option'); option.value = indication.name; option.textContent = indication.name; select.appendChild(option); }); select.value = selectedItem.indication; select.addEventListener('change', () => { const indication = getIndication(antibiotic, select.value); if (indication) { selectedItem.indication = indication.name; selectedItem.clinical = { ...indication }; } renderPrescriptions(); }); label.appendChild(select); return label; }
function buildCard(selectedItem, antibiotic, weightKg) {
  const totalDaily = dailyDoseForAntibiotic(antibiotic, weightKg); const perDose = perDoseAmount(antibiotic, weightKg); const recommended = recommendedFormulation(antibiotic, weightKg); const card = document.createElement('article'); card.className = 'prescription-card';
  const removeBtn = document.createElement('button'); removeBtn.type = 'button'; removeBtn.className = 'remove-button'; removeBtn.textContent = 'Remove'; removeBtn.addEventListener('click', () => { state.selected = state.selected.filter((item) => item !== selectedItem); renderPrescriptions(); });
  const title = document.createElement('div'); title.className = 'card-header'; title.innerHTML = `<h3>${antibiotic.name}</h3>`; title.appendChild(removeBtn); const meta = document.createElement('div'); meta.className = 'card-meta'; meta.innerHTML = `<span>${antibiotic.route}</span><span>${antibiotic.interval}</span><span>${antibiotic.frequency}-times daily</span>`;
  const indication = buildIndicationSelect(antibiotic, selectedItem); const top = document.createElement('div'); top.className = 'card-top'; top.append(title, meta, indication);
  const dosePanels = document.createElement('div'); dosePanels.className = 'dose-panels'; dosePanels.append(buildDosePanel(selectedItem.incoming, 'Incoming Prescriber Dosing'), buildDosePanel(selectedItem.clinical, 'Clinical Dosing'));
  ...
  const note = document.createElement('p'); note.className = 'card-note'; note.textContent = antibiotic.notes; card.append(top, dosePanels, dosingInfo, note); return card;
}
function renderPrescriptions() { prescriptionList.innerHTML = ''; if (state.selected.length === 0) { prescriptionList.innerHTML = '<div class="empty-state">No antibiotics added yet. Select a medication to build the prescription.</div>'; return; } state.selected.forEach((selectedItem) => { const antibiotic = antibioticCatalog[selectedItem.key]; if (antibiotic) prescriptionList.appendChild(buildCard(selectedItem, antibiotic, getWeightKg())); }); }
...
function exportPrescriptionSummary() { const lines = ['Pediatric Antibiotic Calculator', '==============================', `Weight: ${formatNumber(getWeightKg())} kg`, `Age: ${formatNumber(Number(ageInput.value) || 0)} years`, notesEl.value ? `Notes: ${notesEl.value}` : '']; state.selected.forEach((item) => { const antibiotic = antibioticCatalog[item.key]; const dose = item.incoming; lines.push(`- ${antibiotic.name} (${item.indication}): ${formatNumber(Number(dose.dose))} ${dose.doseUnit}, ${dose.frequencyUnit === 'hours' ? `q${formatNumber(Number(dose.frequency))}h` : dose.frequencyUnit} for ${formatNumber(Number(dose.duration))} days`); }); const blob = new Blob([lines.filter(Boolean).join('\n') + '\n'], { type: 'text/plain;charset=utf-8' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'pediatric-prescription-summary.txt'; link.click(); URL.revokeObjectURL(url); }
function toggleWeightUnit() { const currentWeight = Number(weightInput.value); if (Number.isFinite(currentWeight) && currentWeight > 0) weightInput.value = formatNumber(state.weightUnit === 'kg' ? currentWeight * 2.20462 : currentWeight / 2.20462); state.weightUnit = state.weightUnit === 'kg' ? 'lbs' : 'kg'; weightUnit.textContent = state.weightUnit; weightToggle.textContent = state.weightUnit === 'kg' ? 'Switch to lbs' : 'Switch to kg'; weightToggle.setAttribute('aria-label', `Switch weight to ${state.weightUnit}`); renderPrescriptions(); }
weightInput.addEventListener('input', renderPrescriptions); weightToggle.addEventListener('click', toggleWeightUnit); addBtn.addEventListener('click', addSelectedAntibiotic); printBtn.addEventListener('click', () => { if (state.selected.length) exportPrescriptionSummary(); else window.print(); }); populateAntibioticOptions(); renderPrescriptions();
