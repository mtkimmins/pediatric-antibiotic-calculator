...
function createNumberInput(labelText, value, step, onInput) {
  ...
  input.type = 'number'; input.min = '0'; input.step = step; input.value = value;
...
function buildDosePanel(dosing, headingText) {
  ...
  const totalBox = document.createElement('div'); totalBox.className = 'total-quantity';
  totalBox.innerHTML = '<span>Total quantity</span><strong></strong>';
  const total = totalBox.querySelector('strong');
  const updateTotal = () => { total.textContent = `${formatNumber(calculateTotalQuantity(dosing))} mg`; };

  const doseRow = document.createElement('div'); doseRow.className = 'dose-input-row';
  const dose = createNumberInput('Dose', dosing.dose, '0.1', (value) => { dosing.dose = value; updateTotal(); });
  doseRow.appendChild(dose.label);
  const doseUnit = document.createElement('label'); doseUnit.innerHTML = '<span>Unit</span>';
  ...
  doseUnitSelect.value = dosing.doseUnit;
  doseUnitSelect.addEventListener('change', () => { dosing.doseUnit = doseUnitSelect.value; strength.label.style.display = dosing.doseUnit === 'ml' ? '' : 'none'; updateTotal(); });
  doseUnit.appendChild(doseUnitSelect); doseRow.appendChild(doseUnit); panel.appendChild(doseRow);

  const strength = createNumberInput('Strength (mg/mL)', dosing.strength, '0.1', (value) => { dosing.strength = value; updateTotal(); });
  strength.label.style.display = dosing.doseUnit === 'ml' ? '' : 'none'; panel.appendChild(strength.label);
  const frequencyRow = document.createElement('div'); frequencyRow.className = 'dose-input-row';
  const frequency = createNumberInput('Frequency', dosing.frequency, '0.1', (value) => { dosing.frequency = value; updateTotal(); });
  frequencyRow.appendChild(frequency.label);
  const frequencyUnit = document.createElement('label'); frequencyUnit.innerHTML = '<span>Schedule</span>';
  ...
  frequencyUnitSelect.value = dosing.frequencyUnit;
  frequencyUnitSelect.addEventListener('change', () => { dosing.frequencyUnit = frequencyUnitSelect.value; frequency.input.disabled = ['BID', 'TID', 'QID'].includes(dosing.frequencyUnit); updateTotal(); });
  frequencyUnit.appendChild(frequencyUnitSelect); frequencyRow.appendChild(frequencyUnit); panel.appendChild(frequencyRow);
  frequency.input.disabled = ['BID', 'TID', 'QID'].includes(dosing.frequencyUnit);
  const duration = createNumberInput('Duration (days)', dosing.duration, '1', (value) => { dosing.duration = value; updateTotal(); });
  panel.appendChild(duration.label); updateTotal(); panel.appendChild(totalBox);
  return panel;
}
...
function buildIndicationSelect(antibiotic, selectedItem) {
  ...
  antibiotic.indications.forEach((indication) => { const option = document.createElement('option'); option.value = indication.name; option.textContent = indication.name; select.appendChild(option); });
  select.value = selectedItem.indication;
  select.addEventListener('change', () => {
    const indication = getIndication(antibiotic, select.value);
    if (indication) { selectedItem.indication = indication.name; selectedItem.clinical = { ...indication }; }
    renderPrescriptions();
  });
...
function buildCard(selectedItem, antibiotic, weightKg) {
  ...
  const removeBtn = document.createElement('button'); removeBtn.type = 'button'; removeBtn.className = 'remove-button'; removeBtn.textContent = 'Remove';
  ...
  const meta = document.createElement('div'); meta.className = 'card-meta'; meta.innerHTML = `<span>${antibiotic.route}</span><span>${antibiotic.interval}</span><span>${antibiotic.frequency}-times daily</span>`;
  const indication = buildIndicationSelect(antibiotic, selectedItem);
  const top = document.createElement('div'); top.className = 'card-top'; top.append(title, meta, indication);
  const dosePanels = document.createElement('div'); dosePanels.className = 'dose-panels';
  dosePanels.append(buildDosePanel(selectedItem.incoming, 'Incoming Prescriber Dosing'), buildDosePanel(selectedItem.clinical, 'Clinical Dosing'));
  const dosingInfo = document.createElement('div'); dosingInfo.className = 'dose-info';
  ...
  const note = document.createElement('p'); note.className = 'card-note'; note.textContent = antibiotic.notes;
  card.append(top, dosePanels, dosingInfo, note); return card;
}
function renderPrescriptions() {
  prescriptionList.innerHTML = '';
  if (state.selected.length === 0) { prescriptionList.innerHTML = '<div class="empty-state">No antibiotics added yet. Select a medication to build the prescription.</div>'; return; }
  state.selected.forEach((selectedItem) => { const antibiotic = antibioticCatalog[selectedItem.key]; if (antibiotic) prescriptionList.appendChild(buildCard(selectedItem, antibiotic, getWeightKg())); });
}
function addSelectedAntibiotic() {
  ...
  state.selected.push({ key, indication: indication.name, incoming: { ...indication }, clinical: { ...indication } }); renderPrescriptions();
}
function exportPrescriptionSummary() {
  ...
  state.selected.forEach((item) => { const antibiotic = antibioticCatalog[item.key]; const dose = item.incoming; lines.push(`- ${antibiotic.name} (${item.indication}): ${formatNumber(Number(dose.dose))} ${dose.doseUnit}, ${dose.frequencyUnit === 'hours' ? `q${formatNumber(Number(dose.frequency))}h` : dose.frequencyUnit} for ${formatNumber(Number(dose.duration))} days`); });
  const blob = new Blob([lines.filter(Boolean).join('\n') + '\n'], { type: 'text/plain;charset=utf-8' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'pediatric-antibiotic-prescription.txt'; link.click(); URL.revokeObjectURL(url);
}
function toggleWeightUnit() {
  const currentWeight = Number(weightInput.value); if (Number.isFinite(currentWeight) && currentWeight > 0) weightInput.value = formatNumber(state.weightUnit === 'kg' ? currentWeight * 2.20462 : currentWeight / 2.20462);
  state.weightUnit = state.weightUnit === 'kg' ? 'lbs' : 'kg'; weightUnit.textContent = state.weightUnit; weightToggle.textContent = state.weightUnit === 'kg' ? 'Switch to lbs' : 'Switch to kg'; weightToggle.setAttribute('aria-label', `Switch weight to ${state.weightUnit === 'kg' ? 'pounds' : 'kilograms'}`); renderPrescriptions();
}
weightInput.addEventListener('input', renderPrescriptions); weightToggle.addEventListener('click', toggleWeightUnit); addBtn.addEventListener('click', addSelectedAntibiotic); printBtn.addEventListener('click', () => { exportPrescriptionSummary(); window.print(); }); populateAntibioticOptions(); renderPrescriptions();
