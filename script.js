...
function buildIndicationSelect(antibiotic, selectedItem) { const label = document.createElement('label'); label.className = 'indication-field'; label.innerHTML = '<span>Indication</span>'; const select = document.createElement('select'); select.className = 'indication-select'; antibiotic.indications.forEach((indication) => { const option = document.createElement('option'); option.value = indication.name; option.textContent = indication.name; select.appendChild(option); }); select.value = selectedItem.indication; select.addEventListener('change', () => { selectedItem.indication = select.value; const indication = getIndication(antibiotic, select.value); Object.assign(selectedItem, { dose: indication.dose, doseUnit: indication.doseUnit, strength: indication.strength, frequency: indication.frequency, frequencyUnit: indication.frequencyUnit, duration: indication.duration }); renderPrescriptions(); }); label.appendChild(select); return label;
}
function buildCard(selectedItem, antibiotic, weightKg) {
  const card = document.createElement('article'); card.className = 'prescription-card'; const removeBtn = document.createElement('button'); removeBtn.type = 'button'; removeBtn.className = 'remove-button'; removeBtn.textContent = '×'; removeBtn.addEventListener('click', () => { state.selected = state.selected.filter((item) => item !== selectedItem); renderPrescriptions(); });
  const title = document.createElement('div'); title.className = 'card-header'; title.innerHTML = `<h3>${antibiotic.name}</h3>`; title.appendChild(removeBtn); const meta = document.createElement('div'); meta.className = 'card-meta'; meta.innerHTML = `<span>${antibiotic.route || 'PO'}</span>`;
  const indication = buildIndicationSelect(antibiotic, selectedItem); const top = document.createElement('div'); top.className = 'card-top'; top.append(title, meta, indication); const dosePanels = document.createElement('div'); dosePanels.className = 'dose-panels'; const prescribedDosing = buildDosePanel(selectedItem, 'Prescribed dosing'); dosePanels.appendChild(prescribedDosing); const referenceDosing = buildDosePanel(antibiotic, 'Reference dosing', { range: clinicalRange(antibiotic, weightKg) }); dosePanels.appendChild(referenceDosing); updateIncomingStatus(prescribedDosing, selectedItem, antibiotic, weightKg);
  const prescription = document.createElement('div'); prescription.className = 'prescription-content'; prescription.appendChild(dosePanels); const dosingInfo = document.createElement('div'); dosingInfo.className = 'dose-info'; dosingInfo.innerHTML = `<p><strong>Reference total daily dose:</strong> ${formatNumber(dailyDoseForAntibiotic(antibiotic, weightKg))} mg</p>`;
  prescription.appendChild(dosingInfo); card.appendChild(top); card.appendChild(prescription); return card;
}
function renderPrescriptions() { prescriptionList.innerHTML = ''; if (state.selected.length === 0) { prescriptionList.innerHTML = '<div class="empty-state">No antibiotics added yet. Select a medication and click Add.</div>'; return; } const weightKg = getWeightKg(); state.selected.forEach((item) => { const antibiotic = antibioticCatalog[item.key]; const card = buildCard(item, antibiotic, weightKg); prescriptionList.appendChild(card); }); }
function addSelectedAntibiotic() { const key = selectEl.value; if (!key || state.selected.some((item) => item.key === key)) return; const indication = antibioticCatalog[key].indications[0]; state.selected.push({ key, indication: indication.name, dose: indication.dose, doseUnit: indication.doseUnit, strength: indication.strength, frequency: indication.frequency, frequencyUnit: indication.frequencyUnit, duration: indication.duration }); renderPrescriptions(); }
function exportPrescriptionSummary() { const lines = ['Pediatric Antibiotic Calculator', '==============================', `Weight: ${formatNumber(getWeightKg())} kg`, `Age: ${formatNumber(Number(ageInput.value))}`, '', 'Clinical notes:', notesEl.value, '']; state.selected.forEach((item) => { const antibiotic = antibioticCatalog[item.key]; lines.push(`${antibiotic.name}:`); lines.push(`  Dose: ${item.dose} ${item.doseUnit}`); lines.push(`  Frequency: ${item.frequency} ${item.frequencyUnit}`); lines.push(`  Duration: ${item.duration} days`); lines.push(''); }); window.print(); }
